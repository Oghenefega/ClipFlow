...
## Current State
- **`dist/ClipFlow Setup 0.2.0-alpha.1.exe` is the fresh installer** (Phase B's first promotion; daily driver was 0.1.9-alpha.5 with none of B1–B4). Fega installs via the in-app "Install update" banner or by running the exe directly; Settings → bottom should read **v0.2.0-alpha.1** afterwards.
- **#166 CLOSED (`status: untested`)** — commit a66b787, full diagnosis + verification on the issue. Remove the label when Fega confirms boxes appear without resizing.
