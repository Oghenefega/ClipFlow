...
- **A sound source stops on the `playing` flag, never on `video.paused` read from a sibling effect (s240).** Effects run in declaration order; the stems' stop call ran above the effect that calls `video.pause()`, saw a still-playing element and left the audio running with the rAF loop already gone. Put the stop in the same effect as `pause()` (right after it) or pass the intent explicitly. Any new sound source (stems, SFX, music) is checked on EVERY stop path: pause button, Space, end of clip, seek while paused — not just on following the picture.
- **Range sliders are scoped to context** (e.g. ±5s around the segment, clamped to neighbor bounds), not the full video duration.
- **Adding nowrap content (tags, pills, labels) to a drawer panel inside a Radix ScrollArea widens the whole drawer past its edge (s265, #444).** The viewport's `display: table` wrapper sizes to the widest row, and there is no horizontal scrollbar, so the overflow is simply clipped. After the change, measure `viewport > div` width against the viewport. If it is wider, add `[&_[data-radix-scroll-area-viewport]>div]:!block` to THAT ScrollArea, never `ui/scroll-area.tsx` (the #215 rule in clipflow-ui-debug). A DOM probe says the tag exists; only a screenshot shows it cut off.
