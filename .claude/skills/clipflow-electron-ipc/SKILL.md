...
- **A scan-time id is not an identity (#214, session 139).** `asset_${Date.now()}_${rand}` is minted when a file is absorbed, so any rebuild of the index re-mints every id while paths stay put — and an `assetId` copied onto a clip then resolves to nothing ("Asset not found" on every save; `markAssetUsed` failing silently). Anything written into a document the USER keeps (clip, project, queue entry) must be keyed on something derived from the referent — a path or content hash — not on the scan that happened to run that day. Where random ids are already stored, resolve **path first, id second** through ONE shared resolver (`findAssetRef` in assets.js) so the next call site inherits the fallback, and grep every `.find(a => a.id === …)` when a stale-reference bug appears — the one you found is rarely the only one.

- **Full-screen overlays gated on main-process state must survive parent-render churn (session 169, #146).** App.js passes inline callbacks (`onClose={() => …}`) — new identity every render — so a `useCallback([onClose])` chain re-fires the overlay's mount/probe effects on EVERY App re-render. EngineSetupView's re-probe closed the overlay mid-flow the moment the underlying "needed" condition flipped (whisperPythonPath is set BEFORE the model phase finishes). Fix pattern: (1) park the parent callback in a ref (`onCloseRef.current = onClose`) so internal callbacks stay `useCallback([], …)`-stable; (2) the main-process `getState` must report an ACTIVE job separately from `needed` — "no longer needed" and "still running" overlap by design, and reopening mid-job should land on the live phase, not dismiss.
