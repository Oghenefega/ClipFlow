...
0b. **`npm start` runs the PROD profile, which since #329 boots a LIVE PUBLISHER.** The scheduler is main-side now: it starts on boot and ticks within a minute, so any clip whose `scheduledAt` has passed goes out to Fega's real accounts. For publish-adjacent work verify with `CLIPFLOW_PROFILE=dev` (guarded — dev refuses to auto-fire) or the packaged exe against the dev profile, never bare `npm start`. If a prod boot is genuinely needed, grep `clipflow-settings.json` for past `scheduledAt` values FIRST and kill the instance the moment the boot assertion passes. (s218: caught at launch, nothing published, but only because it was noticed.)
0c. **Stop instances by PID or by `electron.exe` — NEVER `taskkill //F //IM Corva.exe`.** Source runs are `electron.exe`; the packaged build AND Fega's installed daily driver both share `Corva.exe`, so an image-name kill takes out his running app too. Capture the PID at launch, or check `tasklist` for pre-existing `Corva.exe` PIDs and spare them. (s218: killed his daily driver mid-session.)
...
- **A lesson about a COMMAND belongs where the command is reached for, not in this checklist.** This section is read at *done* time; a forbidden command is typed *mid-work*, so a correct rule filed here watches the mistake happen. It has now cost two consecutive sessions: the s215 bytes-mode rule (s217) and the s137 asar-extract rule (s218, re-violated despite the bullet below). When distilling, ask "at what moment does this need to be true?" and route to the skill already loaded at that moment — packaging/main-process commands to `clipflow-electron-ipc`, edit-time mechanics to the domain skill. Keep this section for things checked BEFORE done, not things done DURING.
...
- **A scripted multi-site edit asserts every anchor first, and writes last (session 216).** #331 moved seven section boundaries, annotated 17 `<Card>` tags and wrapped six call sites inside a 3098-line file. The pass was written so every step is preceded by a `must(...)` assertion — anchor present, regex matched exactly N times, each title matched exactly one line, nothing double-annotated — and the file write is the LAST statement. The first run failed on one wrong needle (an HTML-escaped `&amp;` where the source has a raw `&`) and exited having written nothing; one string fixed, re-ran clean. Without that shape a bad needle silently half-applies a restructure. Pairs with the bytes-mode rule above: bytes in, assertions throughout, byte probe after.
