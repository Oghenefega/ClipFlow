...
## [Unreleased] — 2026-07-24 (session 127) — 0.4.0-alpha.1: AI titles & captions rebuilt around real published copy (#183)

### Changed
- **Version bumped 0.3.0-alpha.14 → 0.4.0-alpha.1 and the installer cut.** Sizing: minor bump, not an alpha tick. The last two installers were repairs ("clips don't get posted twice any more"); this one adds something the app could not do before — it learns from what you publish, ranks its own examples by how those posts performed, and looks at the actual footage when it writes. New capability, so the minor version moves and the counter resets.
