...
2. **Import & Clip Detection**: User selects a renamed recording in the Recordings tab and clicks "Generate Clips." ClipFlow updates the file's status to `processing` in SQLite, then runs a multi-stage AI pipeline (detailed in §3.4). On completion, status is set to `done` and any pending retroactive renames are applied.
...
The AI pipeline (`src/main/ai-pipeline.js`) is the core differentiator. It uses Claude's vision capabilities to analyze both the transcript and visual frames from a recording. The pipeline runs in 8 stages:

---

#### Stage 0 — File Probe
FFmpeg probes the source file for duration, resolution, FPS, codec. Feeds into cutting strategy and subtitle slicing downstream.

#### Stage 1–2 — Project Creation & Audio Extraction
A project folder is created under `{watchFolder}/.clipflow/projects/{id}/`. FFmpeg rips a **16kHz mono WAV** from the source — this format is required by Whisper and reused for energy analysis.

#### Stage 3 — Transcription (stable-ts via Whisper)
A Python subprocess (`tools/transcribe.py`) runs stable-ts on the extracted WAV. Produces **word-level timestamps** with confidence probabilities. Game-specific vocabulary (game name + streaming slang) is injected as an `initial_prompt` to improve recognition accuracy. Output is a JSON array of segments, each containing an array of `{word, start, end, probability}` objects.

The full transcript is later formatted into an SRT file which feeds Stage 4.

#### Stage 4 — Energy Scoring (`energy_scorer.py`)
A second Python subprocess reads the SRT and source video and computes **RMS dB per transcript segment**. Each segment is assigned one of four energy labels:

| Label | Emoji | Meaning |
|-------|-------|---------|
| EXPLOSIVE | 🔥 | Peak loudness — yelling, hype reactions |
| HIGH | ⚡ | Elevated — raised voice, tension |
| MEDIUM | 💤 | Conversational — normal gameplay commentary |
| LOW | 🔇 | Quiet — looting, menus, near-silence |

Two files are produced next to the source video:
- `{videoName}.energy.json` — Machine-readable array: one object per segment with `rms_db`, `peak_energy` (0–1 normalized), and `energy_label`
- `{videoName}.claude_ready.txt` — The human-readable annotated transcript Claude will receive, e.g.:

```
[🔥 EXPLOSIVE] 00:05:31 — oh my god that's insane!
[💤 LOW] 00:05:34 — just walking around looting here
[⚡ HIGH] 00:05:41 — let's go, let's go!
```

Both files are moved into the project's processing directory after generation.

#### Stage 5 — Frame Extraction (Peak-Energy Frames)
The top 20 segments by `peak_energy` are sorted descending. For each, FFmpeg extracts **one JPEG frame at the midpoint** of that segment (scaled to 1280×720, high quality). These 20 frames become the visual evidence Claude receives.

**This is where signal bias enters the pipeline:** Frame selection is 100% determined by audio loudness. Claude never sees a frame from a quiet clutch play, a deadpan one-liner, a slow tactical moment, or a competitive callout — unless those moments also happened to be loud.

#### Stage 6 — Claude API Call (Clip Detection)
The central reasoning step. A request is sent to `claude-sonnet-4-6` with:

**System prompt** (`src/main/ai-prompt.js`, ~444 LOC) built from 7 sections:
1. Task definition — identify 10–25 clips, return JSON
2. Creator profile — archetype, personality description, signature phrases, ranked moment priorities
3. Game/content context — game name + AI-maintained game profile description
4. Clip selection rules — ordered by creator's moment priority ranking (funny first, or clutch first, etc.)
5. Clip boundary rules — 30–90s duration, no mid-sentence starts, 2–3s reaction padding
6. Output format — JSON schema with required fields
7. Few-shot examples — three-tier blending (see below)

**User message** contains:
- The full `claude_ready.txt` — the annotated transcript with energy labels and timestamps
- Up to 20 JPEG frames (base64-encoded) from peak-energy moments

**What Claude reasons over (current signals):**

| Signal | What It Is | How Claude Sees It |
|--------|-----------|-------------------|
| Full transcript | Every word spoken, with timestamp | In `claude_ready.txt`, line by line |
| RMS energy per segment | Loudness label per transcript line | 🔥/⚡/💤/🔇 prefix on each line in `claude_ready.txt` |
| Visual frames | 20 JPEGs from peak-energy moments | Multimodal image blocks in the user message |

Claude is asked in the prompt to reason beyond energy levels and consider content, humor, narrative arc, and creator personality — but the *evidence* it reasons over has already been filtered by energy at Stage 5. Quiet moments are represented only by text, never by a frame.

**Claude's output:** A JSON array of 10–25 clip objects:
```json
{
  "clip_number": 1,
  "start": "00:05:30",
  ...
  "why": "Near-death 1v3 with full team wipe and genuine panic reaction.",
  "peak_quote": "no way, no way, let's GO",
...
**Provider:** Pluggable via `src/main/ai/llm-provider.js`. Default is Anthropic (`anthropic.js`), with an OpenAI-compatible fallback. Model: `claude-sonnet-4-6`, 120-second timeout, 4,096 max output tokens. Cost is logged per call via `pipeline-logger.js`.

#### Stage 7 — Clip Cutting & Subtitle Slicing
For each Claude-recommended clip: FFmpeg **re-encodes** the segment (`libx264 + AAC`, CRF 18, `veryfast` preset) for frame-accurate cuts. A thumbnail is generated at the midpoint. Word-level subtitles are sliced from the source transcription JSON and offset to clip-local time (0-based timestamps).

#### Stage 7b — Per-Clip Retranscription
Each cut clip is retranscribed with Whisper directly on the clip audio (30–90s is cleaner than slicing from a long recording). This produces more accurate word-level subtitles without hallucination artifacts from long-context transcription.

#### Stage 8 — Project Save
Project written to `project.json`. File metadata status updated to `done` in SQLite. Feedback DB updated. Any pending retroactive renames applied. Result returned to renderer.
...
|---------|-----------|-------|
| Transcripts | `{processingDir}/transcripts/` | `{videoName}.srt` |
| Energy data | `{processingDir}/energy/` | `{videoName}.energy.json` |
| Claude context | `{processingDir}/claude/` | `{videoName}.claude_ready.txt` |
| Frames | `{processingDir}/frames/` | `{videoName}_frame_01.jpg` → `_20.jpg` |
| Logs | `{processingDir}/logs/` | `{videoName}_{timestamp}.log` |
| Clips | `{projectRoot}/clips/` | `clip_001.mp4`, `clip_001_thumb.jpg`, … |

---

**The "loud = important" bias and its two contact points:**

The current pipeline embeds a loudness-centric heuristic in two places:

1. **Frame selection (Stage 5):** Only the 20 loudest segments get visual representation. Claude cannot see what a quiet tactical play or a deadpan joke *looks like* — it only reads about them.
2. **Transcript labeling (Stage 4):** Energy labels in `claude_ready.txt` are purely audio-derived. A whispered clutch moment or a slow strategic explanation gets 🔇 LOW regardless of its content value.

This works well for Hype-archetype creators (loud = important is often true) but underperforms for Competitive, Chill, and Variety archetypes where important moments are frequently quiet.
...
Each file shows: colored game tag badge, filename, file size, and status badge (`renamed`, `processing %`, `done` with clip count). Selecting a file and clicking "Generate Clips" triggers the full AI pipeline (see §3.4), which updates status in SQLite throughout.
...
- **Richer clip detection signals**: Current pipeline uses only 2 signals (RMS energy + transcript). Planned addition of 4–5 new local signals (scene change detection, silence gap analysis, face/body detection, speech rate analysis, and others TBD) to give Claude richer, archetype-agnostic evidence to reason over. This removes the "loud = important" bias.
...
│   │   ├── ai-pipeline.js            # 8-stage clip detection pipeline with SQLite status tracking
...
│   │       ├── instagram-oauth.js   # Instagram-specific BrowserWindow OAuth
...
│   ├── transcribe.py                 # stable-ts wrapper for Whisper transcription
│   └── (energy_scorer.py lives at D:\whisper\ — external, not checked in)
├── tasks/                             # Dev tracking (todo.md, lessons.md)
├── reference/                         # Documentation (this file, architecture notes, specs)
...
- **energy_scorer.py lives outside the repo**: Hardcoded to `D:\whisper\energy_scorer.py`. Not checked in, not portable.
