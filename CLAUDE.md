...
**Product renamed ClipFlow → Corva (2026-08-19, #268).** User-facing name, `productName`, installer, and window titles are Corva. Deliberately KEPT under the old name: `appId` `com.clipflow.app` (upgrade identity — never change), `window.clipflow` bridge, `.clipflow` project dotfolder, store filenames (`clipflow-settings/-tokens/-publish-log`, `clipflow.db`), `CLIPFLOW_PROFILE` env var, "ClipFlow Imports" folder, `clipflow_*` PostHog events, persisted `source: "clipflow"` tracker value, Cloudflare internals. The GitHub repo and the TikTok/Google developer app names still say ClipFlow — their rename is gated behind the trademark opinion (brief Step 4), which is still pending; the TikTok dev app is frozen mid-review and must not be touched. The Meta developer app's display name was changed to Corva by Fega on 2026-09-10 without waiting for the opinion (a one-off, not a lifting of the gate). `%APPDATA%\clipflow` migrates to `%APPDATA%\Corva` on first boot via `src/main/user-data-migration.js`.
