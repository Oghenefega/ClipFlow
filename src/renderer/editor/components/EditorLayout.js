...
function Topbar({ onBack, requireHashtagInTitle = true, onClipRendered, renderJob, onCancelRenderJob }) {
  ...
  // Assemble the render-ready IPC payload from CURRENT store state (unsaved
  // edits included) — shared by doRender and the viewer screenshot so both
  // capture exactly what the preview shows.
  const buildRenderPayload = useCallback(() => {
    // handleSave persists to disk but doesn't update the clip object in Zustand,
    // so read subtitle/caption data straight from the stores.
  ...
  // Viewer screenshot → Shorts thumbnail (session 124). One-frame render at the
  // playhead through the real pipeline — pixel-identical to the final video.
  ...
  const onScreenshot = useCallback(async () => {
    ...
    const { safeClip, safeProject, safeOptions } = buildRenderPayload();
    const result = await window.clipflow.thumbnailCapture(safeClip, safeProject, time, safeOptions);
  ...
  }, [clip, project, buildRenderPayload]);
  ...
  return (
    <div className="h-12 min-h-[48px] flex items-center px-3 border-b bg-card select-none relative">
      ...
      <div className="relative flex items-center gap-2 z-10">
        ...
        {/* Viewer screenshot result (session 124) — same anatomy as the render toast */}
        {screenshotToast && (
          <div className={`absolute top-full right-0 mt-2 w-[340px] rounded-lg border ${screenshotToast.error ? "border-red-500/40" : "border-emerald-500/40"} bg-popover shadow-xl z-50 p-3`}>
            <div className="flex items-start gap-2">
              <div className={`w-4 h-4 rounded-full ${screenshotToast.error ? "bg-red-500/20" : "bg-emerald-500/20"} flex items-center justify-center shrink-0 mt-0.5`}>
                {screenshotToast.error ? <X className="h-2.5 w-2.5 text-red-400" /> : <Check className="h-2.5 w-2.5 text-emerald-400" />}
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-xs font-medium text-foreground">
        ...
        <div className="flex-1 flex overflow-hidden" style={{ minHeight: 0 }}>
          ...
          <ResizablePanelGroup direction="horizontal" autoSaveId="clipflow-editor-hsplit">
            ...
            <ResizablePanel defaultSize={50}>
              <PreviewPanelNew onScreenshot={onScreenshot} />
            </ResizablePanel>
          </ResizablePanelGroup>
