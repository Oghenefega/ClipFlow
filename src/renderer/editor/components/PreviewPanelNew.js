...
import { shouldOfferReframe, resolveClipReframe, resolveSegmentReframe, fitToScreenReframe, captureRegionRect } from "../utils/reframeStyle";
import { buildRenderPayload } from "../utils/renderPayload";
import { makeCompositeScratch, paintReframeComposite } from "../utils/reframeCompositor";
...
export default function PreviewPanelNew() {
  ...
  // #164 scratch canvases for the compositor: stepped-downscale ladder, blur band, seam feather
  const compositeScratchRef = useRef(null);
  if (!compositeScratchRef.current) compositeScratchRef.current = makeCompositeScratch();
  ...
  const paintComposite = useCallback((canvas, rf) => {
    // #457: the painter is shared with the Projects preview (utils/reframeCompositor.js).
    paintReframeComposite(canvas, videoRef.current, rf, compositeScratchRef.current);
  }, []);
