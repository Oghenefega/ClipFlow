...
"snap" (none), "raw" (no refinement at all), or any '+'-joined subset of hubert / vosk /
parakeet (ladder rows).
FULLPASS=1 scores the FULL-RECORDING words instead (projTranscriptionSegs, shifted to the
clip range) — what resolveSubtitles reads for a failed retranscription or an extended clip
(#360); "hubert" then = the full pass's hubert+snap.
...
voters = {"hubert", "vosk", "parakeet"} if mode == "median" else set() if mode in ("snap", "raw") else set(mode.split("+"))
FULLPASS = bool(os.environ.get("FULLPASS"))
...
def source_segments(c):
    """Clip-relative raw segments: the clip's own retranscription, or (FULLPASS) the full-recording
    segments that overlap the clip range, shifted by -startTime."""
    if not FULLPASS: return json.loads(json.dumps(c["clipTranscription"]["segments"]))
    off, dur = c["startTime"], c["endTime"] - c["startTime"]
    out = []
    for s in c["projTranscriptionSegs"]:
        if s["end"] <= off or s["start"] >= off + dur: continue
        ws = [{**w, "start": round(w["start"] - off, 3), "end": round(w["end"] - off, 3)} for w in s.get("words", []) if w["end"] > off and w["start"] < off + dur]
        if ws: out.append({**s, "start": round(s["start"] - off, 3), "end": round(s["end"] - off, 3), "words": ws})
    return out
if FULLPASS: clips = [c for c in clips if c["projTranscriptionSegs"]]
...
for c in clips:
    ...
    raw_segs = source_segments(c)
    if mode == "raw": new_segs, stats = raw_segs, {"method": "raw"}
    else: new_segs, stats = refine_word_timing(raw_segs, a, sr, device=dev, use_whisperx="hubert" in voters,
    ...
    raw = sorted([w for s in source_segments(c) for w in s.get("words", [])], key=lambda w: w["start"])
...
print(f"mode={mode}{' FULLPASS' if FULLPASS else ''} clips={len(clips)} {time.time() - t0:.0f}s methods={methods} stats={tot_stats}")
