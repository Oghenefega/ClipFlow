...
> <!-- DISTILLED-THROUGH: 2026-09-19 (s268) -- Session 268: no new lessons (no corrections; the #449 Rename numbering fix shipped as planned in alpha.10 and Fega confirmed it). The Rename-tab test recipe (scratch watch folder with controlled drop order, the gateway-BYOK frame sniff that a blanked key does not stop, the boot sweep of stamps for absent files) -> memory project_cdp_verification_gotchas (81). Previously s267 -- Session 267: "when a visual ask could mean keep-what-is-there or add-something-new, ask one line before measuring, planning or holding a release" -> memory feedback_ask_before_measuring (Fega: the burned-in captions, not subtitle words; "no crazy science"). The splash/index CDP targets, the moved Projects nav coordinate, the Media library living under projectsRoot and the 8 s screenshot message -> memory project_cdp_verification_gotchas (80). Previously s266 -- Session 266: "a changelog / What's New line states what the code does, never what one sample looked like; 'without X' only when X is actually removed" -> clipflow-code-review (Fega's correction after the wrap). The main-PID tree kill for the dev app and the per-render thumbnail filename -> memory project_cdp_verification_gotchas (79). Previously s265 -- Session 265: "nowrap content added to a drawer inside a Radix ScrollArea widens the drawer past its edge -- measure viewport>div vs the viewport and apply the scoped !block clamp; screenshot before trusting a DOM probe about layout" -> clipflow-editor-patterns (the #215 rule already lived in clipflow-ui-debug, which does not load while building); opening the editor through the onOpenInEditor fiber prop, runtime light-theme checks via data-theme, and the What's New dialog covering a fresh build's screenshots -> memory project_cdp_verification_gotchas (76-78). Previously s264 -- Session 264: "a counterfactual in option text is a behavioural claim - trace X all the way to Y before offering 'if X, Y would happen'" -> clipflow-trace-verify (sub-bullet under the s139 rule); fixture folders/ids need the proj_ prefix, the createRequire + live-cache assertion for main-process probes, and the export-spy / raw-MutationObserver techniques -> memory project_cdp_verification_gotchas (73-75). Previously s263 -- Session 263: "a repro that agrees with the hypothesis gets the most scrutiny — smallest text match, print what was clicked, check the fields that should not change" -> clipflow-trace-verify; the held-button drag trap, the free main-process AI stub and the find-by-text wrapper trap -> memory project_cdp_verification_gotchas (70-72). Previously s262 -- Session 262: "departing from an issue's written plan is asked before building; every surface that prints editable text reads the same string; no control is a no-op on visible input" -> clipflow-code-review; "casing is text, one AA switch read off the text, remembered spelling, all in casing.js" -> clipflow-editor-patterns + memory project_per_line_subtitle_settings (rule 2 rewritten). Previously s261 -- Session 261: "a cited static trace is a hypothesis for a visual/timing bug - walk it to the symptom and watch it happen" -> clipflow-trace-verify; "sed -i in Git Bash is the forbidden text-mode rewrite; check with file, not git diff" -> clipflow-code-review; "a state-mirroring ref has one writer and is never resynced during render" -> clipflow-editor-patterns. Previously s260 -- Session 260: "a fallback is never silent and an unmeasurable path is a bug to file — every AI call, fallback and user action gets a queryable row" -> clipflow-code-review + memory feedback_track_everything; "price a cost gate before recommending it — state the marginal dollars per month, and if trivial there is no gate" -> clipflow-code-review. Previously s259 -- Session 259: no new lessons (no corrections; the Layout drawer density pass shipped as mocked and approved). The Electron-CDP window-resize gap and the drive.js navigation recipe went to memory project_cdp_verification_gotchas. Previously s258 -- Session 258: no new lessons (no corrections; the four render/transcription hardening fixes shipped as planned). The measured 0.5 s render tolerance and the stale-render-vs-drift finding went to memory project_render_verification. Previously s257 -- Session 257: "a source run of the prod profile dirties the tracked data/clipflow.db — stage by path, never git add -A" -> clipflow-code-review; "a user’s description of app state is a symptom report, not a fact — read the live store / project JSON / publish log and date the change against the dated backups before planning" -> clipflow-trace-verify. Previously s256 -- Session 256: "teardown is one consequential action per command — kill, restore and delete are three calls; never chain rm -rf behind other steps" -> clipflow-code-review; the DaVinci-plugin electron.exe trap and the two-clicks-one-tick / first-match-Save gotchas -> memory project_cdp_verification_gotchas + HANDOFF. Previously s255 -- Session 255: "deleting a profile isolates settings, never the library -- assert projectsRoot/watchFolder and the clip count before any test that writes through the app" -> clipflow-trace-verify; "never type a Windows path through a shell into an app under test; build it in the evaluated JS and read it back checking for control chars" -> memory feedback_bash_backslash_collapse. Previously s254 -- Sessions 253 and 254 (two parallel sessions ran this day; both distilled here). Added later in the redesign session, after it cut alpha.3: "print one raw record and assert the exact field names and casing before any aggregate over object fields -- a dedupe/count-distinct over a missing field reports all-identical, failing toward the confident answer instead of erroring" -> clipflow-trace-verify. From session 253's #403 setup failure: "a verification that compares a PROXY (remote file size) instead of the artifact the consumer actually checks (sha256) is the bug, not the safety net" and "a failure handler records the evidence before destroying it, and names the failing item in the user-facing message" -> clipflow-code-review. From the concurrent redesign session: "write the one-sentence physical scene and run the inverse test on your OWN output; look in the product's own best screen before reaching for a reference" -> clipflow-ux-audit; "name WHICH build any 'how it looks today' claim is about (the installed exe is normally behind HEAD) and a visual 'before' is a real screenshot, never a hand-redraw" -> clipflow-trace-verify. Session 253: "write the good/bad tell as a sentence, not a check/cross glyph pair -- Fega read it as two steps to perform" -> clipflow-code-review. Previously s252 -- Session 252: no new lessons (no corrections; the Analytics rebuild shipped as mocked and approved, 0.5.0-alpha.1 was Fega's version call). Two mechanics gotchas went straight to memory: sed -i flips CRLF to LF -> feedback_bash_backslash_collapse; dev:seed overwrites the dev DB with the stale repo copy -> project_db_locations_verification. Previously s251 -- Session 251: "steps for Fega on a third-party dashboard come from that dashboard's own page (no 'Display API' product; 'at least one video, up to 5')" -> clipflow-code-review; "a don't-touch-until-Y rule carries an expiry, verify Y at the source" -> clipflow-code-review + memories project_tiktok_approved / project_corva_rename corrected. Previously s250 -- Session 250: "a brand asset is the file the running app renders, never the first file with the right name (grep the renderer import, git log the candidate, read the rename memory)" -> clipflow-code-review; the browser-tab-pages-follow-the-website-font rule -> memory feedback_dm_sans_only; the Meta-scope-must-be-declared-on-the-use-case fact -> HANDOFF Watch Out For (+ #391). Previously s249 -- Session 249: "a step the user is told to perform is part of the deliverable — probe a dormant OAuth flow's preconditions (app mode, app_domains, scopes on the dashboard) before asking for a reconnect" -> clipflow-code-review; the safeStorage/Local State probe -> memory project_safestorage_probe. Previously s248 -- Session 248: no new lessons (no corrections; the audit and the lightness-only fix shipped as proposed). Previously s247 -- Session 247: "not dark mode, easy on the eyes = a dim LIGHT theme; mock along the axis the user named (ink polarity first)" -> memory project_theme_system (Sunset paragraph); the computer-use Escape gap and the Queue fixture recipe -> memory project_computer_use_app_names / project_cdp_verification_gotchas. Previously s246 -- Session 246: "hand-rolled toContain/notToContain are string-only; a negative assertion on an array passes vacuously — make a new one fail on purpose" -> clipflow-code-review. Previously s245 -- Session 245: no new lessons (no corrections; the plan was approved and shipped as proposed). Previously s244 -- Session 244: "do not mock a hop in the chain you suspect of eating the field" -> clipflow-code-review; "diff two renders frame-by-frame, never threshold brightness" -> clipflow-ffmpeg-media. Previously s243 -- Session 243: "prove a heuristic on the failing sample before the 15-minute scorer" -> clipflow-code-review; "experiment numbers name what was compared and that nothing shipped" -> memory feedback_plan_clarity; the stranded-word facts -> memory project_subtitle_timing_learning -->
