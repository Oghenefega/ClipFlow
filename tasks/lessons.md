...
> <!-- DISTILLED-THROUGH: 2026-09-20 (s269) -- Session 269: three lessons. (1) Fega corrected where the #450 thumbnail slider sat (between Privacy and Description) -> two new rows in .claude/rules/ui-standards.md: a new control in a platform card goes AFTER the text fields it would interrupt, and previews the user must judge are shown readable in place, never as a hover/drag pop-up. (2) Self-caught: the slider preview kept the scrubbed render open and broke the #188 title->file rename (EBUSY, swallowed) -> clipflow-electron-ipc Distilled Lessons (a preview is a reader; canvas + requestVideoFrameCallback, unload when idle; never drawImage at seeked) AND clipflow-code-review section 5 (a new READER of a user file is tested against every rename/overwrite/delete of that file, in its most active state, on a realistic file) AND memory project_video_preview_file_lock. (3) Self-caught: memory feedback_video_cleanup prescribed a cleanup that never runs (ref read inside a useEffect cleanup is null in React 18; proven locally; #451) -> memory corrected, clipflow-code-review section 3 checklist line, clipflow-optimize critical rule extended, and .claude/commands/session-end.md step 2 now says a distilled rule that prescribes CODE is executed once before it is saved. Also new memory project_youtube_shorts_thumbnail_api (thumbnails.set sticks on Shorts for a YPP channel; the solid-colour private-upload probe). Everything below the NEXT-UNDISTILLED marker dated 2026-09-20 or earlier is distilled. -->
