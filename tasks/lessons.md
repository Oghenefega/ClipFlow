...
> <!-- DISTILLED-THROUGH: 2026-09-21 (s270) -- Session 270: three lessons. (1) Fega had to ask what two handoff questions meant ("what limit? why are you asking me?") -> clipflow-code-review's s156 decision-request bullet now also requires "can a user see/feel it" + my recommendation, internal-only items are not his decision, and a relayed HANDOFF line is rewritten first; .claude/commands/session-end.md step 4 says Next Steps questions are written the way they'd be asked in chat. (2) Fega renamed "No thumbnail" to "Auto thumbnail" -> new "Status tag wording" row in .claude/rules/ui-standards.md (name what the viewer gets, the why goes in the hover). (3) Self-caught: the dev copy's outputFolder and testWatchFolder are Fega's real folders, so a testMode render would have landed in Test Footage -> memory project_cdp_verification_gotchas item 82 (repoint three folders; s270 fx451 harness) + its MEMORY.md index line. Also memory project_youtube_shorts_thumbnail_api updated (#452 decision, #453 per-project upload quota). Everything below the NEXT-UNDISTILLED marker dated 2026-09-21 or earlier is distilled. -->
