...
## Engine tests hit an out-of-memory machine: a 15-minute stall, then "bad allocation" (2026-09-22, session 273, self-caught during the #459 verification)
**What happened:** the first real section re-transcribe in the dev app finished Whisper in seconds, then went silent for ~15 min in the word-timing vote and died with no traceback. A by-hand rerun died at once with `MemoryError: bad allocation`. VRAM was fine (8 of 24 GB); host commit had 2.5 GB free because DaVinci Resolve held ~15 GB. Pushing on could have starved Resolve itself, which is Fega's open app. The in-app checks switched to a stand-in `transcribeBatch`. The real engine ran separately once 9.4 GB was free: 33 s, though Parakeet still failed to load and the vote fell back to three voters.
**Why it happened:** I treated the engine as always runnable and never looked at the machine's memory before a heavy run.
**Rule:** before any real engine run, read commit free. Under ~8 GB, don't run: stub the engine, or ask Fega when Resolve can close. Cap by-hand runs with `timeout 300`. Memory: `project_engine_memory_pressure`.

## The first fixture was an old recording whose "voice" track is game audio (2026-09-22, session 273, self-caught when Whisper returned "not a cat, but a cat…")
**What happened:** I took the #459 fixture from the sanctioned Arc Raiders pool (zero approvals) without checking its audio layout. The January file has 4 audio tracks; today's saved setup describes 5, with the voice at index 1. On that file index 1 is game audio, and Whisper turned it into nonsense that first looked like an engine or code failure. It also exposed a real product bug: Re-transcribe used the index blindly, and it now refuses on a mismatch. The fixture moved to a 5-track recording (MC Day1 Pt1, also zero approvals).
**Why it happened:** the fixture rules cover approvals and rejected clips, not the recording's audio layout.
**Rule:** for anything that reads audio by track index, ffprobe the fixture's audio track count and require it to match `audioSetup.trackCount` before testing.

## The new top-bar menu drew under the preview's Fit control while every DOM check passed (2026-09-22, session 273, self-caught from the first screenshot)
**What happened:** the Re-transcribe menu (`fixed`, `z-[100]`) rendered inside the top bar's `relative z-10` container. Existence, item text and synthetic clicks all passed. The screenshot showed the preview's "Fit" control drawn over "Whole clip". A `fixed` element can't escape its ancestor's stacking context. Once portaled to `<body>` (CropDialog's pattern), `elementFromPoint` on the first row returned the menu.
**Why it happened:** I verified the menu by existence, and an existence check can't see stacking.
**Rule:** a new pop-up menu is portaled to `<body>` unless no z-indexed ancestor sits above it. Verify it with `document.elementFromPoint` on its first row plus a screenshot, never by existence alone.

## The Projects layout posters looked right and held ~700 MB (2026-09-21, session 271, self-caught by the planned load test)
**What happened:** the #457 posters decoded each raw 2560x2880 still with `img.decode()` and drew it into a canvas. Every visual check passed: posters matched the editor at the same moments, and so did playback across a cut. The 24-clip load pass then read the page process at 508 MB after one scroll-through and 714 MB after three. Neither a forced `HeapProfiler.collectGarbage` nor leaving the project brought it down. The same stills shown as plain `<img>` elements settle at ~250 MB. Main now scales the still to 1280 px (`clip:posterStill`, `nativeImage`), the renderer paints from `createImageBitmap` and `close()`s it, and the same passes read ~230 MB.
**Why it happened:** a canvas draw needs the full-resolution decode, which Chromium keeps. An `<img>` shown small is decoded near its display size, so the old view never paid this. The first pass alone looked like a capped cache; only the third pass and the forced GC showed it wasn't.
**Rule:** anything that turns a list of large stills into canvases gets a multi-pass memory run (open, scroll every card, leave; three times; then force GC) against the plain-`<img>` baseline before it ships. Full-size stills are scaled in main, never decoded full-size in the page. Memory: `project_canvas_fullsize_still_memory`.

## The #450 preview held the render 2.5 s after every drag (2026-09-21, session 271, self-caught while timing the #454 picker)
**What happened:** timing the release of the moved picker, the video let go exactly 2502 ms after pointer-up: the safety timer, not the settle check. A per-frame recorder showed why. The last frame of a drag is presented while `video.seeking` is still true, `releaseIfSettled` requires `!seeking`, and no frame follows. #450 shipped with the same gap, so a title rename within 2.5 s of a drag kept the old file name. Adding `onSeeked={releaseIfSettled}` brought release to 61-67 ms on a 114 MB render. A held mid-drag probe showed EBUSY, proving the probe could see the lock at all.
**Why it happened:** the s269 check proved the file was released eventually, not how soon, and a timer that always fires looks the same as a check that works.
**Rule:** when a cleanup has both a condition and a backstop timer, measure which one fired (time it against the triggering event). A backstop that fires every time means the condition is broken. A lock probe is only evidence once it has shown the lock, so hold mid-gesture first.

## Two runs of the same build differ at the editor's feathered edges (2026-09-21, session 271, self-caught during the #457 compositor move)
**What happened:** the before/after fingerprint of the editor's layout canvas matched exactly at 7 s but differed at 2 s: 449 pixels, max 15/255, all at the band-to-blur edges. I nearly treated it as a regression from the move. A second run of the NEW build against the first differed by the same amount (up to 361 px, max 15) at the same kind of edges. It's existing run-to-run noise in the compositor (scratch-canvas history under bilinear sampling), not the change.
**Why it happened:** I compared across builds without first measuring how much one build varies against itself.
**Rule:** a before/after pixel comparison needs a same-build baseline first: run the unchanged build twice. Only differences beyond that noise, or outside the regions where it occurs, count as regressions. Report both numbers.

## The dev copy's render folders were Fega's real folders, and a test render would have written into Test Footage (2026-09-21, session 270, self-caught before pressing Queue)
**What happened:** the #451 fixture reused the s269 recipe, which repoints only `projectsRoot`. Before the "press Queue in the editor" test I read where the render would land. A `testMode` project renders to `testWatchFolder\Corva Renders` (`resolveTestAwareOutputFolder`), and any other project renders to `outputFolder`. In the dev profile both were real `W:\` folders (`Test Footage\2026-10` and `Downloaded Clips\ClipFlow Renders`). I repointed both before the test, and a `find` afterwards confirmed nothing landed in the real folders. The dev `watchFolder` is also the real Recordings folder, so the dev copy's Rename tab lists real pending files.
**Why it happened:** the s269 recipe was built for tests that never rendered. The CDP-gotchas memory also said `outputFolder` is isolated per profile, which is no longer true.
**Rule:** before any dev-profile test that can render, repoint `projectsRoot`, `outputFolder` AND `testWatchFolder` to the fixture (s270 `fx451/setup.js` plus its output repoint). Afterwards, search the real folders for the fixture's names. Never click Rename in a dev copy that watches the real Recordings folder.

## Two yes/no questions reached Fega in code-speak, and he had to ask what they meant and why they were his (2026-09-21, session 270, Fega: "explain question 2a better please - what is the youtube size limit? why is it there? why are you asking me?")
**What happened:** the s269 wrap, and my s270 reminder after the alpha.11 cut, put two housekeeping questions to Fega copied almost word for word from the handoff: "fix the outdated upload-limit comment in the YouTube upload file?" and "should a re-queued clip clear an old Instagram '720p' tag the same way it now clears 'No thumbnail'?". The first is a note for developers: a stale YouTube quota figure in a code comment. No user ever sees it, and changing it changes nothing. He read "upload-limit" as a file-size limit and could not tell why the decision was his. The second assumed he knew why re-queuing clears anything at all. So he questioned the design itself ("if there was no thumbnail, shouldn't it still say no thumbnail?").
**Why it happened:** handoff lines are notes to the next session (me), and I passed them to him without translating. The surgical-changes rule (ask before touching code outside the task) was right to apply. But I handed him the decision with no context and no recommendation, so a note with no effect on the app cost him a round trip.
**Rule:** every question to Fega says in plain words (1) what the thing is, (2) whether a user can see or feel it, (3) what changes if he says yes, and (4) my recommendation. If a user can't see or feel it (a code comment, internal naming), it is not his decision: say what I'll do and fold it into the next change unless he objects. Rewrite handoff lines before they reach him.

## The #450 tag was named after the step that failed, not what the viewer gets (2026-09-21, session 270, Fega: "rather than 'no thumbnail' I think it's better phrased as auto thumbnail")
**What happened:** when YouTube refuses the picked frame, the Queue shows a yellow "No thumbnail" tag. But every YouTube video has a thumbnail: when a custom one is refused, YouTube shows its own automatic pick. So the label described a state that never exists on the channel.
**Why it happened:** I named the tag after our failed step (the thumbnail call), not after what the viewer actually sees. The neighbouring "720p" tag already does it right: it says what went out.
**Rule:** a status tag says what the user or viewer actually gets now ("Auto thumbnail", "720p"). Which step failed, and why, goes in the hover text.

## The thumbnail preview kept the rendered file open, and the title edit in the same card stopped renaming it (2026-09-20, session 269, self-caught in the pre-commit review)
**What happened:** the first #450 picker was a paused `<video>` of `clip.renderPath`. It passed every check I had planned: seek, save, restart, hints. Re-reading the diff against the card it lives in, I noticed the same card edits the title, and a title edit renames the render on disk (#188). A first lock test passed (29 MB file, never scrubbed). Only after scrubbing a 145 MB file did the app log `Could not rename … EBUSY`: Chromium keeps a scrubbed local file open on Windows. `renameAssetTo` swallows the error by design, so nothing on screen would ever have said so. The fix (copy each presented frame to a canvas, unload the video once the slider settles) then had its own bug: an off-screen video drawn at `seeked` often has no drawable frame, so backward drags kept the previous picture. That one was caught only because the test video was colour-coded and the test dragged in both directions.
**Why it happened:** the verification plan covered what the new control does, not what it does to its neighbours. A preview is a reader, and I treated readers as harmless. On Windows a reader can block a writer. The first lock test also used the easy case (small file, no scrubbing) and passed, which is the s204/s205 lesson again: the cell that differs most from the one you tested is the one that breaks.
**Rule:** before done on anything that opens a user file (a `<video>`, an `<img>`, a stream, ffmpeg), list every operation elsewhere in the app that renames, overwrites or deletes that same file, and run each of them while the new reader is in its MOST active state (scrubbed, large file), not freshly opened. For frame previews: in-DOM video + `requestVideoFrameCallback`, never `drawImage` at `seeked`; test pictures are colour-coded by time and drags go both ways. Memory: `project_video_preview_file_lock`.

## The memory note for video cleanup prescribed a cleanup that never runs (2026-09-20, session 269, self-caught while writing the picker's cleanup)
**What happened:** memory `feedback_video_cleanup` told every new `<video>` to unload itself with `const vid = videoRef.current` INSIDE the `useEffect` cleanup. React 18 clears DOM refs before passive cleanups run, so on unmount that read is `null` and the unload is skipped. Three components had copied it (ProjectsView clip player, editor MediaPanel, AudioCalibrationModal). I suspected it from React's documented behaviour, then proved it with the repo's own React 18.3.1 in a hidden Electron window (inside-cleanup read: null; captured node: VIDEO) before filing #451 and correcting the memory.
**Why it happened:** the note was written from the crash it fixed, and nobody checked that the prescribed code still ran on the path it was written for. A rule that is copied verbatim gets trusted more each time it is copied.
**Rule:** a memory or skill that prescribes CODE gets that code executed once before it is saved, on the exact path it claims to cover. When a suspicion about existing code rests on framework semantics, prove it locally first (Electron can `require` the repo's own React in a hidden window: no downloads, real DOM), then file the issue with the output in it.

## The thumbnail picker was wedged between Privacy and Description (2026-09-20, session 269, Fega: "it shouldn't be between the title and description the way it is right now. That doesn't make sense.")
**What happened:** for #450 the plan and the mock put the new Thumbnail row directly under Privacy in the Queue's YouTube card, because that is where the other small per-clip YouTube setting lives and where its sibling saver sits in the file. In the mock that split the card's text fields: Title, Privacy, a 128px video preview, and only then Description and Tags. Fega rejected the placement on sight. He also turned down option A's grow-on-drag preview ("looks weird").
**Why it happened:** I placed the control by code adjacency rather than by how the card reads. The card follows the order of the post itself (title, description, tags), and the #291 comment in the same function ("YouTube tags — under the description, not above it") already recorded that Fega orders this card by reading flow. The plan text said "under Privacy" and he approved it, but a line in a plan is not a picture; the mock is where placement actually gets judged.
**Rule:** a new control in a platform card goes AFTER the text fields it would otherwise interrupt. Title → description → tags stay contiguous and extras (thumbnail, option panels) come last, the way the TikTok options panel already does. Decide placement from the rendered card's reading order, not from where the neighbouring saver sits. A preview the user has to judge is shown at a readable size in place, never as a hover or drag pop-up.

## A What's New line described one sampled frame as if it were the feature; Fega read it as "subtitles removed" (2026-09-18, session 266, Fega: "What do you mean, WITHOUT a subtitle word on top of it")
**What happened:** for #446 I pulled frame 0 of ONE real render. Nobody was speaking at 0.0 s in that clip, so no subtitle word was on screen. I turned that into a product claim, "the first frame shows your title card without a subtitle word on top of it", and wrote it into the plan, a code comment, CHANGELOG, HANDOFF and the user-facing What's New. The code only grabs frame 0 of the finished video, with everything burned in. My own test clip's thumbnail showed the subtitle "firing" at frame 0, which contradicted the line, and I read straight past it.
**Why it happened:** a single sample's incidental property was reported as the behaviour. It is the s112 rule ("one probed file proves 'this file WOULD', never 'Z is happening'") applied to copy instead of diagnosis. Worse, the phrasing sounded like a feature that REMOVES something the user needs.
**Rule:** a changelog or What's New line states what the code does ("the thumbnail is the first frame of the finished video, captions and subtitles included"), never what one sample happened to look like. Before shipping a line that says "without X" or "no longer shows X", check that X is actually removed. If it is only absent sometimes, don't write it.

## An option I offered Fega rested on a scenario I had not traced, and he chose it (2026-09-17, session 264, self-caught while reading the Queue filter for the plan)
**What happened:** planning #438, I asked whether Post should also confirm before re-posting a clip that had already gone out. The option text said that if he had clicked "2 minutes later" the in-flight guard would not have saved him, and the clip would have double-posted silently. He picked "Yes, add a confirm". Reading `approved` for the plan showed the scenario can't happen: a full-success publish writes a tracker row, `tracker:appended` reaches App.js, and the Queue's id knockout (QueueView.js:680) drops the card within a second, taking Post with it. The confirm would have guarded a hole that doesn't exist. I dropped it in the plan and said why, but his choice had already been made on a false premise.
**Why it happened:** the s139 rule ("an assertion inside a question is still an assertion") was loaded, but I read it as being about "nothing else does X" claims. A *scenario* ("if you had done Y, Z would have happened") didn't register as a claim, because it came from reasoning rather than memory. It is still a claim about what the app does along a path, and I had only traced half the path. I knew what the guard covers, not what happens to the card after a publish completes.
**Rule:** a counterfactual in option text is a behavioural claim. Before offering "if X, then Y would happen", trace X all the way to Y in the code: what the UI shows at that moment, and what removes or changes it. If it can't be traced in time, leave the scenario out of the option and let the option stand on what is known.

## I reported a bug "reproduced exactly" when the probe had pressed a different button (2026-09-17, session 263, self-caught by a fixture title that should not have changed)
**What happened:** reproducing #437 ("Apply says Applied, caption unchanged") the probe found the caption card with `find(e => e.children.length <= 1 && /text/.test(e.textContent))`. A wrapper with one child also "contains" the text and comes first in document order, so the walk-up to "the card's Apply button" started from a container holding EVERY card and pressed a TITLE card's Apply. The result — card area says Applied, caption unchanged — matched the bug perfectly, and I told Fega it was reproduced. It was only caught a run later because the fixture clip's title had become "Saved title A". Redone with a corrected probe (smallest matching element, and a guard that the chosen card contains no title text) on the old code via `git stash`, then on the fix.
**Why it happened:** the outcome agreed with the hypothesis, so nothing prompted a look at what the click had actually done. A repro that confirms what you expect gets the least scrutiny and needs the most.
**Rule:** a text-located element is the SMALLEST element containing the text (sort matches by `textContent.length`), never the first. After any probe click that is supposed to demonstrate a bug, print what was clicked (the card's own text) and check the fields that should NOT have changed as well as the one that should. "Reproduced" is only said after that.

## A simulated drag that is cut off mid-gesture leaves the mouse button held, and every later gesture lands on the stale drag (2026-09-17, session 263, self-caught with a temporary recorder)
**What happened:** verifying #435, an "All subtitles" drag wrote a single line's position and every drag covered about half its distance. Three clean replays with a recorder in `onSubtitleDragStart`/`onSubtitleYChange` showed the right target every time. The likely cause: an earlier `timeout 25 node d.js drag …` was killed between `mousePressed` and `mouseReleased`, so the page kept `dragging=true` with the previous drag's ref, and later `mouseMoved` events fed it before any new pointerdown.
**Rule:** drive drags from a wrapper that cannot be killed mid-gesture (`execFileSync` with its own timeout, as `seq.js` does), and when a drag result contradicts code just written, relaunch the app and replay before debugging the code. Partial distances are the tell.

## Fega's description of the problem was wrong in both directions — and the live settings file said so in ten seconds (2026-09-14, session 257, self-caught by reading the store before planning)
**What happened:** the session opened with "my 'other platforms' on the queue page have now been broken into categories and games, meaning I can't just change 1 and it'll show across all of them" and a request to add `| FOLLOW FOR MORE!😏` to three platforms. Both halves were wrong. `captionTemplates` is three global strings shared by all 16 library entries (`captionResolve.js:115`) — one edit reaches everything, no per-game anything. And `| FOLLOW FOR MORE!` was already in all three templates, added by Fega himself at 00:11 that morning. Reading `%APPDATA%\Corva\clipflow-settings.json` and diffing it against the dated backups beside it turned a believed 16-edit problem into a 3-string diff whose real delta was one emoji and a missing `!` on Instagram.
**Why it happened:** nothing — this one went right. The pull was to accept the premise ("broken into categories and games") as a fact about the code and start designing a bulk-edit path for a structure that does not exist. What stopped it was opening the data first: the store, the dated `clipflow-settings.backup-*.json` files, and `clipflow-publish-log.json` (whose 2026-09-12 captions proved the line had appeared since).
**Rule:** a user's description of app state is a symptom report, not a fact — before planning against it, read the live artefact that decides the behaviour (the store file, the project JSON, the publish log) and the dated backups beside it, and say plainly when the premise doesn't hold. On this project the settings store keeps its own history in `clipflow-settings.backup-*.json` / `.bak-*`: diffing today's file against them dates any change and names who made it. Correcting the premise out loud IS part of the deliverable — the 16-edit version of this task was never real work.

## A source run of the prod profile rewrote the tracked repo DB, and `git add -A` swept 330KB of binary into the feature commit (2026-09-14, session 257, self-caught by reading `git status` output after the commit)
**What happened:** verifying #417 meant `npx electron .` on the prod profile. Running from source puts the database at `<repo>/data/clipflow.db` (CLAUDE.md says so) instead of `<userData>\data\`, and boot ran a title/caption backfill — 125 rows inserted — growing the tracked file from 245,760 to 577,536 bytes. `git add -A` staged it and the commit went in with three files. Amending with `--only <paths>` did NOT drop it; the fix was `git checkout HEAD~1 -- data/clipflow.db` then a plain `--amend`.
**Why it happened:** `data/clipflow.db` is tracked, stale, and never thought about — the memory about it (`project_db_locations_verification`) is phrased as "don't measure against it", which is about reading, so writing to it didn't register as a consequence of `npm start`.
**Rule:** any source run of the prod profile dirties `data/clipflow.db`. Stage by path (`git add CHANGELOG.md src/...`), never `git add -A`, in any session that launched the app from source — and check `git show --name-only HEAD` before pushing. To drop a path from a commit already made: `git checkout HEAD~1 -- <path>` then `git commit --amend --no-edit`; `--amend --only <other paths>` does not remove it.

## The cleanup script bundled a kill, a settings restore and an `rm -rf` into one command — and Fega declined the whole thing (2026-09-13, session 256, permission denied)
**What happened:** after the layout verification I wrote one Bash line that killed the dev app by PID, restored the dev settings snapshot, and deleted the scratch project root. The permission prompt showed an opaque wall of PowerShell, `node`, and `rm -rf`, and Fega said no. Split into three plain commands (kill; restore; leave the folder), the first two went through immediately and the third was simply not needed — the scratch root lives in the session scratchpad.
**Why it happened:** the steps were sequentially related in my head, so they became one command; the existing rule ("one consequential action per command … never bundle file-writes + rm -rf into one opaque script", `.claude/docs/issue-filing.md`) was written for gh closes and I didn't apply it to teardown.
**Rule:** teardown is one consequential action per command. Kill, restore and delete are three separate calls, each readable on its own line of the prompt, and a delete of anything outside the repo is proposed as its own step or skipped when the location is already disposable. If a command would need a sentence to explain what it destroys, it is two commands.

## A "virgin" profile on Fega's machine is not a fresh customer — and I spent minutes clicking around 167 of his real clips believing it was empty (2026-09-11, session 255, self-caught by a "0 clips" expectation that returned 167)
**What happened:** to verify #406/#407 against a true new install I moved `%APPDATA%\clipflow-dev` aside and let the app recreate it. The Rename tab then showed a `W:\...\Vertical Recordings Onwards` watch folder and the Queue showed **167 clips with real titles** — on a profile created seconds earlier. Two independent mechanisms did it: `main.js:501` (#251) pins the legacy `W:` path into any store that has no `watchFolder` **whenever that path exists on disk**, which is always true on Fega's machine and never on a customer's; and `libraryRoot()` (`main.js:267`) falls back `projectsRoot || watchFolder`, so the empty profile went straight to reading his real `.clipflow` project tree. I had already clicked the Queue tab twice before the clip count registered as impossible.
**Why it happened:** "I deleted the profile" felt like proof of isolation, so the emptiness was assumed rather than asserted. The seeding is also deliberate and correct — there is no bug to notice, which is exactly why it doesn't announce itself. The near-miss is the shape of session 143: operating on shared real data while believing the sandbox is empty, one destructive click away from damage.
**Rule:** deleting a profile isolates SETTINGS, never the LIBRARY — project data lives on disk under `projectsRoot`/`watchFolder` and a fresh store will happily point at it. Before any test that writes through the app, assert the library is the fixture you intended: print `projectsRoot`, `watchFolder` and the clip count, and require the count to be the number you expect (0 for a fresh-install test). Repoint `projectsRoot` at a scratch directory rather than trusting a recreated profile. And on this machine, fresh-install behaviour that depends on a path existing (`fs.existsSync` guards) cannot be reproduced at all — say so instead of reporting it as verified.

## The path I wrote into the store arrived with a 0x0F control character in it (2026-09-11, session 255, self-caught by reading the value back)
**What happened:** setting a test `outputFolder` through CDP, I passed a Windows path with escaped backslashes through the Bash heredoc. It stored as `C:UsersIAMABS~1...(0x0F)9c7f5b...` — separators gone, and a literal \17 eaten into the byte 0x0F. The dependency-check assertion still passed, because a corrupt non-empty string is as truthy as a real one, so only reading the value back caught it.
**Why it happened:** the known backslash-collapse (`feedback_bash_backslash_collapse`) applies to CDP expression arguments too, which is a hop I hadn't associated with it — the string survives bash, JSON and JS unscathed right up until one layer eats a level.
**Rule:** never type a Windows path through a shell into an app under test. Build it in the evaluated JS — `['C:','Users',...].join(String.fromCharCode(92))` — and read the value back asserting `!/[\x00-\x1F]/.test(v)` before trusting any result that depends on it. A truthiness check on a path proves nothing about the path.

## "Reconnect once" was handed to Fega before the reconnect had ever been run (2026-09-10, session 249, Fega: "hmmn? I'm trying to reconnect")
**What happened:** the Analytics tab needs two new Meta permissions, so the plan said "Fega reconnects Instagram and Facebook once". I verified the chip renders and the fetchers fail with the right permission error, and shipped alpha.33. The reconnect itself died in the browser: Facebook's "Can't load URL — domain not in the app's domains", because the Meta app had gone to Live mode since the accounts were first connected and a localhost return address is only accepted in Development mode. Meanwhile the chip made it easy to click the second Meta button while the first waited, and both share port 8083 — eleven "address already in use" alerts. He ended with BOTH accounts disconnected and no way back, on the daily driver, with clips scheduled.
**Why it happened:** the OAuth flow had not run in months, so "it worked when the accounts were connected" was stale evidence; the dashboard is state outside the repo that the code cannot see. I verified my new UI and my new fetchers, not the pre-existing step the user was being told to perform.
**Rule:** a step the user is told to perform is part of the deliverable and gets verified like code. For a dormant OAuth flow that means probing its preconditions before the ask — `GET /{app-id}?fields=app_domains` with the app token, the redirect URI against the app mode, and whether the new scopes are added on the dashboard — and never asking for a reconnect that begins with a disconnect on the machine that publishes. Promoted to `clipflow-code-review`.

## Five assertions passed while testing nothing — the hand-rolled matcher type-guards itself into a no-op (2026-09-07, session 246, self-caught by a sibling matcher throwing)
**What happened:** the new reject-reason chip tests asserted on arrays: `expect(chips).toContain("not-funny")`, `expect(chips).notToContain("teaches-nothing")`. The repo's hand-rolled runner (`src/main/ai-prompt.test.js:52-77`, copied into `game-profiles.test.js` and `segmentWords.test.js`) defines `notToContain` as `if (typeof actual === "string" && actual.includes(sub)) throw` — so on an array the guard short-circuits and the assertion **passes unconditionally**. Every `notToContain` I wrote against an array was vacuous. I only found out because its sibling `toContain` guards the opposite way (`typeof actual !== "string" || …` → throws), and it failed with the baffling "Expected string to contain not-funny" while printing an array that visibly contained it. Had I written only the negative assertions, a green suite would have shipped proving nothing.
**Why it happened:** the matcher names read like Jest's, which accept arrays. Nothing about `expect(chips).notToContain(x)` looks wrong at the call site, and a passing test is the one signal nobody re-examines.
**Rule:** in this repo's hand-rolled runners, `toContain`/`notToContain` are STRING-ONLY. Assert on arrays by joining with delimiters (`"," + keys.join(",") + ","`, matched as `",key,"`) so both directions actually run, or write an explicit loop that throws. More generally: before trusting a new negative assertion, make it fail once on purpose — a `notToContain` that cannot fail is worse than no test, because it looks like coverage. Promoted to `clipflow-code-review`.

## The unit test mocked the exact hop that was eating the field (2026-09-07, session 244, self-caught by the e2e probe)
**What happened:** #374 was "a disabled subtitle line still burns into the video". I added the `enabled !== false` filter to `resolveTimelineSubtitles` after its two branches merge, wrote six unit tests, and they all passed. The real render was unchanged. The resolver branch maps four named fields (`start`, `end`, `text`, `words`) out of each segment, so `enabled` was already gone by the time my filter ran — and my test had `jest.mock`ed `resolveClipSubtitles`, the very hop that drops it, returning objects that still carried the flag. The test was asserting my filter works *given* the field survives, which was the thing in question. Only the frame-diff probe on the exported MP4 caught it; the fix had to move to ingestion inside each branch.
**Rule:** when the bug is "a field disappears somewhere along a chain", the test must not mock any hop in that chain — mocking the suspect is assuming the answer. Either drive the real implementation with real data, or verify at the output. A green test whose fixture hand-carries the field proves nothing about whether the field survives.

## A burn-in probe that thresholded brightness could not see a whole subtitle line (2026-09-07, session 244, self-caught after two false FAILs)
**What happened:** to prove the disabled line was gone from the export I counted near-white pixels (>235 gray) in the subtitle band, control render vs disabled render. Both read ~3,300 and the probe said FAIL twice. The band is bright from the gameplay underneath on every frame — a full subtitle line is about 1% of that count, well inside frame-to-frame noise. I nearly concluded the fix was broken when it was already correct. Switching to a direct frame difference between the two renders (same clip, same encoder, deterministic) gave 1,870 changed pixels inside the line's 0.4 s window and exactly 0 everywhere else, including 1 s either side.
**Rule:** verify an overlay/burn-in change by diffing two renders of the same clip frame by frame, never by thresholding absolute brightness against live footage. The diff needs no assumption about where the element sits, what colour it is, or how it is styled, and its "0 changed pixels everywhere else" half is what proves nothing ELSE was disturbed. Promoted to `clipflow-ffmpeg-media`.
...
**Why it matters:** each failure burned a round trip and, worse, the heredoc one could have half-written a file if the quote had broken later in the body. The bash-backslash memory already said "large heredocs go through the Write tool"; this is the same rule from the quoting side.
**Rule:** a `node -e` one-liner or heredoc body that contains an apostrophe, or a command that mixes a heredoc with `'$(…)'`, is written to a file with the Write tool and run from there. The reusable helpers (`scripts/dev/cdp.js`, `cdp-shot.js`, `hunks.js`) are committed so they are never retyped through the shell again. Promoted to memory `feedback_bash_backslash_collapse` (s238 note).


## `"..."` in a repeated schema slot means "inherit"; replacing it with prose silently drops the spec (2026-08-31, session 224, self-caught in review)
**What happened:** fixing #344, I rewrote the title/caption JSON schema's caption 2 and 3 placeholders from `"..."` to `"<a DIFFERENT angle from title 2 — not a rewording of it>"`. Card 1's slot carries the full format spec (4-9 words, first person, no hashtags); `"..."` had been inheriting it. My prose replaced the inheritance instead of adding to it, so the model read the word budget as card-1-only — and every word-budget violation in the verification runs landed on **card 2** (`7,9,4 | 6,10,4 | 5,5,4 | 5,10,4`). Restating the spec on both slots dispersed it back to baseline.
**Why it matters:** it shipped in the first commit and passed a build, a boot and three live generations. The tell was not the violation count (which actually improved) but its POSITION — a per-card clustering that only means something if you look at where failures land, not how many there are.
**Rule:** in any repeated schema, spec table or few-shot block, `"..."`/"same as above" is load-bearing. Before replacing one instance with real text, list what the sibling slots are inheriting and carry it across. When verifying a multi-slot generator, bucket failures BY SLOT — a uniform rate and a clustered rate mean opposite things.

## A prompt's display notation leaks into its output unless the output rules explicitly forbid it (2026-08-31, session 224, the #343 bug itself)
**What happened:** `formatVoice` flattens each example caption's real newline to `" / "` so the shape fits one prompt line, and the header said `" / " marks the line break`. Nothing in the OUTPUT FORMAT block said how to *write* a break, so the model imitated the notation and emitted a literal slash — which Fega then saw burned into caption suggestions. The training data was clean (70 published captions with a real newline, 0 with a slash); the prompt manufactured the bug.
**Rule:** any notation introduced to *display* examples (a separator, an ellipsis, a bracket convention, a truncation marker) must be negated in the output rules — "this mark is notation, never write it" — and the real encoding stated positively ("a real `\n` inside the JSON string"). Assume few-shot examples teach their formatting as strongly as their content.

## A rule added to a prompt needs a visible exemplar, or it teaches nothing (2026-08-31, session 224, self-caught in review)
**What happened:** I added a caption rule that a blank line (`\n\n`) sets a second beat apart — backed by real data (a good share of published captions use one). But `flattenCaption`'s single regex collapsed `\n\n` and `\n` to the same `" / "`, so every example of the shape I was describing rendered identically to a plain two-line caption. The rule was asserted with nothing beneath it to point at.
**Rule:** after adding a rule to a prompt, re-render the prompt with real data and find the examples that demonstrate it. If none are distinguishable, either fix the rendering so they are (here: stanza breaks became `" // "`) or drop the rule. A rule with no exemplar is prompt weight without prompt signal — exactly what the #183 rebuild was written to remove.

## Two eyeball claims about the data, both overturned by measuring (2026-08-31, session 224, self-caught)
**What happened:** (1) I flagged the caption word budget as writing against Fega's habit, citing a couple of long captions I'd noticed — measuring said 4-9 words covers **79 of 81** published captions, median 6. The rule was right; only the LINE-count rule was wrong (2 lines mandated, but 12 are three-line and 11 are one-line). I had bundled a correct rule and an incorrect one into a single impression. (2) Later, two word violations in a verification run looked like a regression I'd introduced — an A/B against the pre-fix prompt put the old one at 3/12 and the new at 2/12. Pre-existing drift, not my regression.
**Rule:** before claiming a rule contradicts the data, count it; before calling a post-change observation a regression, run the pre-change code on the same input. Both are one script against the prod DB or one `git show <sha>^:<file>` away. An eyeball impression can be half-right in a way that sends a fix at the wrong half.

## Text-matched CDP clicks lied three ways in one drive; a screenshot fixed all of it (2026-08-30, session 222, self-caught)
**What happened:** driving the editor/Settings for the #333/#334 verification, `/back/i` clicked "Feed**back**" (opened the feedback widget), an exact-text "Settings" matcher clicked an element in a pane that wasn't fronted (the editor was full-window), and several probes assumed a left sidebar — the nav is a **bottom bar**. Ten blind probes; one `Page.captureScreenshot` showed the whole layout and every subsequent click landed.
**Rule:** whole-word matchers on trimmed text, candidates filtered by visibility AND expected region, and after one click that doesn't change the page, screenshot before probing again. Re-confirms the s109 screenshot-first and s108 visibility-scoping traps; routed the sharpened version to [[project_cdp_verification_gotchas]] (trap 60).

## The migration plan trusted its own story about who writes clip.gameTag; the store said otherwise (2026-08-30, session 222, self-caught, caught BEFORE writing)
**What happened:** the approved plan said "clear the one stray per-clip `gameTag: "Val"` override from the dropdown experiment". Reading the real project.json during pre-apply verification showed EVERY clip in all 8 projects carries a per-clip `gameTag` ("JC") — left as-is, clip-level JC would have beaten the new project show-tag in every resolution path and the migration would have shipped looking complete while changing nothing user-visible in the Queue. Plan amended to retag all 123 clip-level tags.
**Why it matters:** this is s212's "a spec's description of a stored value is a claim; the store is the fact" working as designed — the read-the-store step fired and caught it. Re-confirmation, no new routing.

## A button wired to `onMouseDown` is invisible to a synthetic `.click()` (2026-08-30, session 221, self-caught)
**What happened:** driving the Queue tag editor over CDP, `[...buttons].find(b => b.textContent.trim()==='Cancel').click()` reported "clicked" and did nothing — twice, across two probe rounds, before I looked at the JSX. That Cancel is wired `onMouseDown={e => { e.preventDefault(); … }}` **on purpose**: mousedown-with-preventDefault is how the app stops a click from blurring the field into a save. `.click()` dispatches only click, never mousedown, so the handler never runs.
**Why it matters:** this pattern is now everywhere the tag/caption editors live (`TagInput`'s pill ✕ and Clear all, the Queue's Cancel buttons) and it will spread with any future blur-sensitive control. Gotcha 15 covers synthetic-vs-trusted in general; this is the specific, recurring shape — and it fails SILENTLY, looking like a broken feature rather than a broken probe.
**Rule:** before concluding a button does nothing, grep the JSX for how it is wired. `onMouseDown` handlers need `Input.dispatchMouseEvent` press+release at real coordinates, not `.click()`. Corollary: a control that deliberately avoids blur is a tell that the surrounding feature is blur-sensitive — test the blur path too. Routed to [[project_cdp_verification_gotchas]].

## A CDP `keyDown` carrying `text` PLUS a separate `char` event types everything twice, and bypasses preventDefault (2026-08-30, session 221, self-caught)
**What happened:** the first pass of the tag-editor drive produced `zzzzzztteesstt` for `zzztest` and left a literal `,` sitting in the field after the comma-commit. My key helper sent `keyDown` with `text` AND a follow-up `type:'char'` event — each inserts the character, and the standalone `char` is delivered regardless of the keydown handler calling `preventDefault()`. Read naively, that says "comma doesn't commit, it just types a comma" — a fake bug in brand-new code, which is the worst place to have one.
**Rule:** dispatch `keyDown` with `text`/`unmodifiedText` set and then `keyUp` — nothing else. That fires the handler and performs the insertion as one unit, so `preventDefault()` suppresses the character the way a real keypress does. Reserve a bare `char` event for cases where no key handler is involved. Sanity-check any new input driver against a known-good field before trusting its output. Routed to [[project_cdp_verification_gotchas]].

## Ran `asar extract-file` from the repo root again — the exact command a memory warns about (2026-08-30, session 221, self-caught, no damage)
**What happened:** verifying what shipped inside the alpha.12 asar, I reached for `npx asar extract-file dist/…/app.asar src/main/release-notes.js /dev/stdout` with the repo as CWD. `extract-file` writes to the BASENAME in the current directory — which is precisely how a past session silently overwrote `package.json` with the stripped packaged copy ([[project_package_json_strip]]). It no-opped this time (no stray file, `package.json` intact, verified via `git status` + a `scripts` key check), so nothing broke.
**Why it matters:** the knowledge existed and was written down; I did not consult it before typing a familiar-looking command. The existing memory already prescribes the safe alternative, so there is no new rule to route — the gap was recall, not knowledge.
**Rule (re-confirmation, not new):** verify packaged contents by reading the asar bytes (`grep`/Python `in`), never `extract-file`. That is what I switched to, and it answered the question in one call.


## Alpha suffixes string-glued onto T colours became invalid CSS the day T became var() strings (2026-08-28, session 220, self-caught)
**What happened:** the s220 audit of the alpha.10 batch found EIGHTEEN sites across eight files
composing semi-transparent colours as `` `${T.yellow}12` `` / `` `${T.cyan}88` `` — valid 8-digit
hex before #328, but `var(--yellow)12` after it. A declaration containing var() only fails at
computed-value time, so the browser drops it SILENTLY: banner tints, engine-setup warning boxes,
Tracker glow dots, Projects player glows and both new Settings info boxes all shipped flat in
alpha.10 with no error anywhere. The #328 sweep converted ~307 colour literals but missed every
concat site because a concatenation doesn't LOOK like a colour literal — and the #329 commit,
written after themes, added two more using the same dead pattern.
**Why it matters:** this is the sibling of s217's var()-in-non-CSS-consumers lesson. That one was
"var() strings reaching a consumer that can't resolve them"; this one is "var() strings reaching a
STRING COMPOSITION that assumes hex". Both classes survive build, boot, and a passing smoke test.
**Rules:** (a) never compose an alpha by appending to a `T` value — use the `Dim`/`Border` tokens
for washes/borders, `color-mix(in srgb, var(--x) N%, transparent)` for exact alphas (verified
resolving in Electron 40); (b) after ANY token migration, grep the concat classes explicitly:
`\$\{T\.\w+\}[0-9a-fA-F]{2}` and `T\.\w+\s*\+\s*"`. Routed to `clipflow-ui-debug` (Distilled
Lessons) + one line in `.claude/rules/ui-standards.md` (fires at edit time).

## Log lines attributed by tail recency belonged to the previous session's boot (2026-08-28, session 220, self-caught)
**What happened:** 12 seconds after launching a dev verification boot, `tail`ing app.log showed
"Re-added 1 trackerData entr(ies)" and I spent ~15 minutes chasing a phantom anti-clobber bug in MY
boot — the lines belonged to session 218's boot five hours earlier; my boot's lines had not flushed
yet. The `sess_` id on every line would have said so immediately. (The "orphaned" XP entries that
deepened the scare were s218's documented delete-test residue, discoverable in its changelog entry.)
**Rules:** (a) attribute app.log lines by matching the `sess_` id to the boot you launched, never by
recency — a fresh boot's lines can flush after your first read; (b) before chasing residue found in
a shared profile's store, read the recent session wraps/changelog for a harness that deliberately
created it. Routed to `clipflow-trace-verify` (Distilled Lessons).

## The release build OOMs at the packaging step, and the fix is a bigger Node heap (2026-08-28, session 219, self-caught)
**What happened:** `npm run build` for the alpha.10 cut failed after a clean `vite build` with
`RangeError: Array buffer allocation failed` at `NtExecutable.generate` (`pe-library`), reached
via `addWinAsarIntegrity` -> `beforeCopyExtraFiles` in app-builder-lib. electron-builder rewrites
the whole ~200 MB `Corva.exe` PE image in memory to stamp the asar-integrity resource, and that
single allocation now sits close enough to Node's default heap ceiling to tip over. Re-running
verbatim with `NODE_OPTIONS=--max-old-space-size=8192` succeeded with no other change.
**Why it matters:** the failure is at the LAST step, after the renderer has already built
successfully, so a partial read of the log looks like a healthy build. `dist/` is left holding the
PREVIOUS version's installer and manifest, so publishing without checking artifact timestamps
would silently re-ship the old version under a new version number in `package.json`.
**Rules:** (a) run the release build with `NODE_OPTIONS=--max-old-space-size=8192`; (b) never infer
success from `vite build` finishing - the packaging phase is a separate failure domain; (c) step 4
of the release loop (verify fresh timestamps + the manifest's `version:` line) is what catches this,
so it is not skippable. Routed to `clipflow-update-launcher` (Gotchas + step 3).

## A backgrounded build does not survive the session process exiting (2026-08-28, session 219, self-caught)
**What happened:** the release build was started with `run_in_background: true` per the skill's own
instruction. The session process exited while it ran; the task came back `stopped` with "No
completion record was found", and the build had in fact died. Roughly four minutes of wall clock
bought nothing, and the state was ambiguous until the output file was read.
**Why it slipped:** backgrounding is the right call for a build whose result you will poll later
IN the same session, and the skill's advice ("takes a few minutes, run it in the background") was
written from that assumption. A release build is not that shape - nothing else can usefully proceed
until it finishes, because every remaining step consumes its artifacts.
**Rule:** run `npm run build` for a release cut in the FOREGROUND with a long timeout (600000 ms;
it takes ~3-5 min). Background only work that genuinely runs alongside something else. Routed to
`clipflow-update-launcher` (step 3).

## "Open" and "untested" are not "unshipped" - verify against git ancestry, not a prior handoff (2026-08-28, session 219, user-flagged)
**What happened:** HANDOFF.md said "The live feed is `0.4.0-alpha.9`. **Five** issues now sit behind
one cut: #325, #324, #331, #328 and #329." Two of those (#324, #325) had shipped IN alpha.9 -
commit 19c2122 is literally titled "cut installer to promote the Queue redesign". The line
contradicted itself: if the feed is alpha.9, alpha.9's contents are by definition not behind the
cut. Fega read the handoff, saw my changelog credit three issues, and asked why I had dropped two.
**Why it slipped:** the count was taken from open `status: untested` issues. That label tracks
whether FEGA has confirmed a fix, not whether the code reached a build - the two drift apart the
moment an installer ships something he has not yet exercised.
**Rules:** (a) an installer is a full build of master, never a delta - anything merged is in it, so
"which issues does this promote" means "which landed since the last bump", answered by
`git log <last-bump>..HEAD`; (b) to check whether a specific fix already shipped, use
`git merge-base --is-ancestor <commit> <bump-commit>`, not the issue board; (c) to check whether
Fega has RUN a version, read `lastSeenVersion` in `%APPDATA%\clipflow` settings - it is stamped
only by `whatsnew:ack`, i.e. only when he closed the What's New screen; (d) never repeat a prior
handoff's count without re-deriving it. Routed to `clipflow-update-launcher` (step 2 + Gotchas).

## A colour-token substrate leaks into three consumers that cannot resolve CSS variables (2026-08-28, session 217, self-caught)
**What happened:** #328 replaced ~307 hardcoded colours with `var(--...)` tokens across the renderer.
Three of the rewritten sites were not CSS: a `<canvas>` 2D gradient in PreviewPanelNew
(`addColorStop("rgba(var(--shade),...)")` throws - the 2D context has no custom properties), the
caption drop-shadow fallback in `subtitleStyleEngine.js` (also `require`d by
`subtitle-overlay-preload.js`, which runs in a separate window with no themed root), and a scrim
printed over video frames with white text on it. All three build, minify and ship; two would have
failed silently and one would have thrown inside the preview render.
**Why it slipped:** a mechanical sweep matches on the SHAPE of a value (`rgba(...)`), which is
identical whether the string is destined for a stylesheet, a canvas API, an FFmpeg argument, or a
second BrowserWindow. "Is this a CSS value?" is a question about the consumer, not the literal.
**Rules:** (a) before any mechanical colour/style sweep, enumerate the NON-CSS consumers of the
files in scope - grep `getContext("2d")`, `require(".*renderer/` from `src/main/`, and any second
BrowserWindow's preload - and exclude or hand-audit them; (b) after the sweep, grep the changed
tree for `var(--` and confirm every hit lands in a `style={{}}`, a className, or a stylesheet;
(c) a token that reaches video content or a brand fill is a literal by definition, not an
oversight. Routed to `clipflow-code-review` (Pre-Build checklist, CSS/Layout section).

## The bytes-mode rule existed, was written for exactly this, and I still hit it (2026-08-28, session 217, self-caught)
**What happened:** every scripted sweep this session used `io.open(p, encoding='utf-8')` to read.
Text mode translates CRLF to LF on READ, so writing back with `newline=''` wrote pure LF - 42
source files converted, the same failure s215 already wrote a rule about. `git diff` showed
nothing (autocrlf normalises both sides; the diff stayed 742/481), so the usual check was blind
again. Caught only when the code-review checklist was read at the END of the work, and repaired
in bytes mode.
**Why it slipped:** the rule lives in `clipflow-code-review`, which by name and by habit runs
BEFORE BUILD - i.e. after every sweep has already been written. A constraint on HOW to perform an
edit is useless in a checklist consulted after the edits exist.
**Rule:** scripted source edits open in binary, always - `open(p,'rb')` -> replace on bytes ->
`open(p,'wb')`. Not "when a sweep is large enough to warrant it": make it the only shape used, so
there is no judgement call to skip. Then match the file's EXISTING endings rather than imposing
CRLF - lessons.md is pure LF while the JS tree is CRLF, and an assertion on that is what caught
the first attempt at this very entry. Verify with a byte probe against an untouched sibling,
never `git diff`. Corollary: when a lesson constrains the METHOD of an edit, it belongs in the
skill that fires when the work STARTS, not the one that fires at done.

## I derived a card index from grep'd headings and one "card" turned out to be two headings in one box (2026-08-28, session 216, self-caught)
**What happened:** building the #331 Settings search index, I mapped the six accordion groups by grep'ing the card-title pattern (`fontSize: 14, fontWeight: 700`) and treated every hit as one card. That produced 24 entries and a rail that advertised "Games 4". The truth is 23: **Games** and **Content Types** are two headings inside a single `<Card>` (one opens at :952, the next `<Card>` is at :999 with both titles between). Only a later grep of `<Card` *interleaved* with the titles showed the gap. Caught before shipping, but only because I happened to print `<Card` and the titles in the same listing; the count would otherwise have gone out wrong on a visible label, and two index entries would have fought over one DOM anchor id.
**Rules:** (a) a heading is not a container — when a text search is the basis for a count, an index, or a set of anchors, grep the CONTAINER opening tag in the same pass and pair them up; N titles is an upper bound on N boxes, never a confirmation; (b) derive user-visible counts from the deduplicated container set (`new Set(...map(i => i.id)).size`), not from the index length, so a shared container can carry two search names without inflating the label; (c) the live probe is what settled it — reading the rendered count back out of the running app is the cheapest proof that a structural assumption survived contact. Routed -> [[clipflow-trace-verify]] (Distilled Lessons).

## An assertion-guarded script made a 3100-line restructure safe to attempt (2026-08-28, session 216, self-caught, worked)
**What happened:** #331 needed seven section boundaries moved, 17 `<Card>` tags annotated and six call sites wrapped inside a 3098-line file — too mechanical for hand edits, too structural to do blind. I wrote one Python pass in **bytes** mode (per the s215 rule) where every single step is preceded by a `must(...)` assertion — anchor present, regex matched exactly 5 times, each title matched exactly 1 line, no Card double-annotated — and the file write is the LAST statement. First run failed on `Title &amp; Caption Style Guide` (I had HTML-escaped the ampersand; the source has a raw `&`) and exited before writing a single byte. Fixed one string, re-ran, clean. Follow-up byte probe confirmed 0 control characters.
**Rule:** for any scripted multi-site source edit, assert every anchor up front and make the write the last statement — a failed assertion then costs a re-run, not a damaged file or a half-applied restructure. Pair it with the s215 rules: bytes mode only, byte probe afterwards. Routed -> [[clipflow-code-review]] (Distilled Lessons).

## I rewrote a source file with Python text-mode open() and silently flipped 3150 lines from CRLF to LF (2026-08-28, session 215, self-caught)
**What happened:** doing a 4-occurrence style replacement in QueueView.js, I reached for a Python heredoc using `io.open(p, encoding="utf-8").read()` / `.write()` instead of the Edit tool. Text mode does universal-newline translation on read (CRLF → LF) and writes back plain LF, so the whole file's line endings changed as a side effect of a 4-line edit. `git diff --stat` still showed 38/15 because the repo normalises to LF and git hid it, so the damage was invisible in the place I would normally check. Caught it only because I ran a separate byte-level line-ending probe out of caution; restored CRLF with a bytes-mode round-trip and re-verified the diff. The existing memory ([[feedback_bash_backslash_collapse]]) already said "NEVER write source via text-mode open()" — I knew the rule and used the tool anyway because this edit "felt too small" for the Edit tool.
**Rules:** (a) for source files, use Edit/Write — a scripted rewrite is only for genuinely mechanical multi-file sweeps, and then in **bytes** mode (`open(p,'rb')` → replace on bytes → `open(p,'wb')`), never text mode; (b) `git diff --stat` does NOT reveal line-ending damage when the repo normalises — verify with a byte probe (`d.count(b'\r\n')`) after any scripted write; (c) an existing memory rule is not weakened by the edit being small — "too small to bother" is exactly the reasoning the rule exists to overrule. Routed -> [[clipflow-code-review]] (Distilled Lessons).

## I flagged the exact twin of the bug I was fixing instead of fixing it (2026-08-26, session 210)
**Correction:** #315's real fix was in `retryFailed` — a stamp that got overwritten and a tracker entry derived from the wrong moment. `publishClip`, in the same file, held a character-for-character copy of both. I fixed one, wrote the other up as a "noted, not changed" caveat on the issue and in the handoff, and moved on. Fega: "fix the publishClip twin too." The tell was in my own hands the whole session: #321, two commits earlier, was fixed on the explicit principle that twins which must agree should be ONE function — and the reason it had a bug at all is that a UI gate and a store check had drifted apart.
**Rules:** (a) when the bug you are fixing has an exact twin in the same file or module, fixing one and flagging the other is not "surgical" — it ships a known bug and leaves two copies to drift; fix both in the commit, or collapse them into one function; (b) scope discipline means not inventing NEW work, not declining the other half of the work you just found; (c) if the twin genuinely must wait (different risk, different verification), say so and ask in the same breath rather than filing it as a caveat. Routed -> [[clipflow-code-review]] (Distilled Lessons).


## The changelog-heading blunder I flagged in review, I then committed myself one edit later (2026-08-26, session 209, self-caught)
**What happened:** the s209 review of 446d6f3 found that the commit had "updated" the top `## [Unreleased] — (session 207)` heading into the session-208 one instead of inserting a new section — erasing s207's record and stranding its entries under the wrong heading. I restored the heading, wrote it up as a finding... and while adding the session-209 changelog entry minutes later, used the s208 heading as my Edit anchor and replaced it the exact same way. Caught it on my own re-check one edit later and restored s208's heading too. Two authors (Opus, then Fable), same session-day, same mechanism: the top heading is the natural-looking Edit anchor for "add an entry at the top", and Edit replaces anchors — it doesn't prepend.
**Rule:** a new changelog session section is INSERTED — anchor the Edit on content that survives (the old heading goes in the new_string too, below the new section), and after any changelog edit re-read the top to count one heading per session. Routed -> [[clipflow-code-review]] (Distilled Lessons).

## A parity fix verified only on its target case shipped a render that never finishes (2026-08-26, session 204, self-caught)
**What happened:** s203's review fix flipped the media-overlay stage from `eof_action=pass` to `eof_action=repeat` so a play-once GIF freezes on its last frame instead of vanishing. That was proven against real FFmpeg -- on a play-once GIF. But `eof_action` governs every overlay on that stage, and the two kinds it was never run against are ENDLESS inputs: a still is `-loop 1`, a forever-looping GIF is `-ignore_loop 0`. With `repeat`, the overlay stage keeps producing output after the picture is done and ffmpeg runs until it is killed. Measured: a 10s clip with one still overlay was still encoding at 60s and 3.6 MB, vs exit-0 in 0.3s with `pass`. It shipped in 62ee3ee, passed a fresh-eyes review, and was found a session later only because #311's own multi-input harness hung. Fixed by capping every looping overlay input at the timeline duration (`-t <timelineDuration>`), which ends it with the picture and leaves the freeze behaviour untouched (re-proved frame by frame).
**Rules:** (a) a fix that changes a SHARED flag/option/default is not verified until the cases it did NOT target have been run -- enumerate the other inputs that option applies to, name what makes each different, and test the one that differs most; (b) "the streams I tested all end" is an assumption about the INPUT, not about the filter -- when a filter option is about end-of-stream, the input that never ends is the case; (c) a render harness that times out is a result, not a flake. Routed -> [[clipflow-ffmpeg-media]] (Distilled Lessons, next to the existing eof_action line) + [[clipflow-code-review]] (Distilled Lessons).
...
## Session 205 — enumerate the untargeted cases; measure duration, not termination

s204 followed s204's own lesson (re-verified the shared eof_action flag on a still and a
play-once GIF) and still shipped two export-overrun bugs, because "the cases the fix didn't
target" was answered by intuition instead of enumeration. The missed shapes sat on axes nobody
listed: placement TIME (looping GIF at tlStart>0 — its -t cap ignored the setpts shift, 8s clip
→ 11.9s file) and length VS CLIP (video window past the timeline's end — overlay shortest=0
repeats the main's last frame, 8s clip → 30.0s file, and that's the DEFAULT drop of a long
reaction clip). Both passed "it finished, exit 0"; only ffprobe format=duration exposed them.

**Rule:** for any overlay/flag/graph change, write the shape matrix first (media kind ×
placement time 0/mid × length shorter/longer than clip) and probe output duration per cell run.
**Deeper rule:** when the requirement is about the OUTPUT (export length), enforce it on the
...
## S212 — the spec said "lowercased tag"; the store says `RL`, `EO`, `SCoG`

**What happened:** #322's spec (written by me last session) described a game tag as "the lowercased
short tag from gamesDb". Building on that, `setAssetGame` and `importAssets` both normalised the tag
with `.toLowerCase()` before storing it. Opening the real dev profile to set up verification showed
gamesDb actually holds `AR`, `RL`, `Val`, `EO`, `DD`, `PoP`, `SCoG`, `Pico`, `JC`, `MC`, `BB` — and
clips carry them unchanged. Every stored assignment would have been `rl` while every comparison
looked for `RL`: the feature would have built, shipped, and silently matched nothing. Caught before
the first run only because verification setup requires reading the real store.

**Why it slipped:** a spec I wrote is the most trusted document in the room and the least verified —
the casing claim was never checked against data, just carried forward from one session to the next.

**Rule:** a spec's description of a data shape is a claim, not a fact. Before writing any code that
compares, normalises or keys off a stored value, open the real store/DB and look at the actual
...
## S218 — `taskkill //F //IM Corva.exe` kills Fega's daily driver, not your test instance

**What happened:** verifying the packaged build meant launching `dist/win-unpacked/Corva.exe`.
Shutting it down with `taskkill //F //IM Corva.exe` matched the image name, which is shared
with the INSTALLED app — so it also force-killed the copy Fega had been running all session.
No data loss (the app persists on every change) but it is his daily driver and he had to
reopen it.

**Why it slipped:** source runs are `electron.exe` and packaged/installed runs are `Corva.exe`.
The whole session had used `taskkill //IM electron.exe` safely, and the same shape was reached
for without noticing the image name had become the shared one.

**Rule:** never `taskkill //F //IM Corva.exe`. `electron.exe` is safe (only source runs use it);
`Corva.exe` is not (the installed daily driver shares it). To stop a packaged instance you
started, capture its PID at launch and kill that PID, or check `tasklist` for pre-existing
`Corva.exe` PIDs first and spare them.


## S218 — a rule in a done-time checklist does not fire at do-time (the asar-extract repeat)

**What happened:** ran `npx asar extract-file ... package.json` from the repo root, which
overwrites `package.json` with the stripped packaged copy — scripts, devDependencies and the
whole `build` block gone. This is documented in memory `project_package_json_strip` AND already
sits in the `clipflow-code-review` skill as its own bullet ("when a memory or lesson names a
command as forbidden, its named substitute is not optional"). The rule existed, was correct,
and did not fire.

**Why it slipped:** the bullet lives under **Distilled Lessons**, a section read at *done* time
— after the code is written, before the commit. The forbidden command is typed *mid-work*,
while investigating. Same failure shape the s217 marker already recorded for the s215
bytes-mode rule: a correct rule in the wrong-timed section is a rule that watches the mistake
happen. Two consecutive sessions have now lost to this same structural problem.

**Rule:** a lesson about a COMMAND belongs where the command is reached for, not in a
done-time checklist. Promoted the asar rule into `clipflow-electron-ipc` (the skill loaded for
main-process and packaging work) as an inspection-commands line, so it is in context before the
command is typed. General form: when distilling, ask "at what moment does this need to be true?"
and route to the skill that is already loaded at that moment — the code-review checklist is for
things checked BEFORE done, not for things done DURING.

**Second-order:** also worth knowing for the recovery — `git checkout -- package.json` restores
...
## Session 232 (2026-09-02) — a one-estimator ceiling is not the ceiling

**What happened:** s231 wrote "the honest ceiling for automatic timing on this data is 82-83%" after scoring four aligners one at a time (each alone was worse than raw stable-ts). s232 took the per-word MEDIAN of three of them and landed 84.6% — above every single aligner including the one that cannot ship — with the collateral of the gated rule. The number I had called a ceiling was the ceiling of one estimator.

**Why:** I generalized from "each tool disagrees with Fega on ~17-20% of words" to "the residual is taste" without checking whether the tools disagree with Fega on the SAME words. They didn't; independent errors average out.
...
## Session 241 (2026-09-05) — a timer is not a paint; verify what shipped, not what was sent

**What happened:** Fega's eight posts went out without their opening title card (#363). The overlay capture waited a fixed 20 ms after changing the DOM and then photographed the offscreen window; under load the photo was the previous (empty) frame, and the identical-frame cache then replayed that empty photo for the whole card. It only bites when nothing animates under the card, which is why weeks of speaking-from-frame-0 clips looked fine and today's hype-reaction clips did not.
...
## Session 250 (2026-09-10) — the first logo file you find is not the logo

**What happened:** Asked to put the Corva logo on the sign-in result pages, I grabbed `public/icon.svg` — a purple square with a lightning bolt — and built the mock around it. Fega: "have you forgotten what the new corva logo actually is now?" The current mark is `src/renderer/assets/brand/clipflow-mark.png` (swapped in by #252, `c0d5b8a`, together with `public/icon.png`/`.ico`); `icon.svg` is the pre-#252 leftover nobody deleted.

**Why:** I searched by filename ("logo", "icon"), took the only SVG as the canonical source because SVG is the convenient format to inline, and never checked what the app itself renders. A stale file with the right name beat the real asset because I optimised for the format I wanted instead of for the truth.
...
## Session 254 (2026-09-10) — the redesign was the category reflex wearing a skill's name

**What happened:** asked to redesign Corva's UI using impeccable + interfaces + ui-ux-pro-max, I produced a before/after mockup and Fega's verdict was "it's using a lot of generic and AI slop so far." He was right. My "after" was a segmented control with muted count numerals, ghost buttons at 1px/8px, a status line joined by middots, and a sortable platform table with share bars. Linear/Vercel/Stripe defaults, top to bottom. I had impeccable's AI-slop test and its category-reflex check loaded in context and did not run either one on my own output.

**Why:** I ran the skills as a *checklist against the existing UI* (find the banned side-stripe, find the hero-metric tile) and then filled the holes with whatever the training distribution hands you for "dense desktop product tool." Passing the ban list is not the same as having a point of view. impeccable says this in as many words: the inverse test is to describe what you are about to build the way a competitor would describe theirs, and restart if the sentence fits the modal product in the category. Mine — "a dense tool with a segmented filter bar, a compact status line and a sortable table" — is every creator-SaaS competitor Corva has.

**The miss underneath it:** I never wrote the scene sentence impeccable requires before choosing anything, so I never noticed that Corva is a *cull/triage* tool (hundreds of keep-or-kill judgments a week on video) dressed as a dashboard. Its own Analytics tab already solves this — real frames, big, number burned on — and the screens where the work actually happens abstract 18 videos into 18 grey pips. The direction was inside the app the whole time.

**Rule:** before mocking anything, write the one-sentence physical scene (who, where, what light, what mood, doing what, how often) and run the inverse test on the result out loud. Then look for the answer in the product's own best screen before reaching for a reference. A design that could carry any other product's logo is a failed design, however many bans it clears.

## Session 254 (2026-09-10) — I designed against a screenshot of a stale build, twice removed

**What happened:** I presented before/after redesign mockups. Fega: "these 'before' versions are not indicative of the current version of the app. What version are you using?" He was right on two separate counts.

1. **Wrong version.** I screenshotted the running installed exe, which is `0.5.0-alpha.1`. The repo is at `0.5.0-alpha.2`, three commits ahead, and one of them (#401) rewrote 193 lines of `AnalyticsView.js` — the exact tab I built a whole "before" panel for and proposed replacing. The About screen I screenshotted said **"Update available — 0.5.0-alpha.2"** in the second line. It was in the first image I captured and I read straight past it.
2. **Not the app at all.** Even for the tabs that had not changed, the "before" panels in both mockup files were *my hand-written HTML recreations* of what I remembered from a screenshot. A recreation is my interpretation with my spacing and my type sizes. Presenting it as "Before — recreated from today's build" invited exactly the trust it did not deserve: any detail I got wrong becomes a fake problem I then "solve".

**Why:** I treated "I took a real screenshot" as sufficient grounding and stopped checking. The version was on screen; the installed-vs-source split is written in CLAUDE.md in its own table; `git log` since the installer commit takes one command. I did none of it because the screenshots *felt* like ground truth.

**Rule:** before any before/after work, pin the target build explicitly and say which it is — installed exe or repo HEAD — and run `git log <installer-commit>..HEAD -- src/renderer/` to list what the installed copy is missing. For a visual "before", use the **actual screenshot** as the artefact, or run the current source and screenshot that; never hand-redraw a surface and label it "before". If a recreation is genuinely unavoidable, label it as a sketch and say what it was traced from.

**Corollary:** the daily driver being one version behind HEAD is the normal state of this project, not an anomaly. Any claim about "how the app looks today" is ambiguous until that gap is named.

## Session 253 (2026-09-10) — the publish step verified a proxy, so it verified nothing (#403)

**What happened:** Fega's laptop failed the setup wizard at "Subtitle timing" with "a timing model didn't verify — it may have been corrupted in transit." Nothing was corrupt. `publish-runtime.ps1` skipped any model upload whose **remote size** already matched, and its post-publish verification checked **Content-Length**. `build-models.ps1` rebuilds every zip on each run, and a rebuild differs only in a zip timestamp extra field — identical size, different sha256. So R2 kept the 03:35 objects while the always-reuploaded manifest carried the 04:30 hashes. The app's checksum was correctly refusing a perfectly good download, on every machine, forever; "Try again" could never have worked.

**Why:** the publisher compared what was cheap to compare, not what the consumer checks. Two green checkmarks ("[SKIP] matching size", "[OK] size verified") described a state that was never verified. Confirmed the hosted and local zips differ in exactly two bytes, at offsets 95 and 96.

**Rule:** when a downstream consumer verifies X, the producer must compare X or something that provably implies it. For S3/R2 that is the multipart ETag, recomputable locally from the file with no download — now used for both the skip decision and the post-publish assertion. Interrogate every "already up to date / unchanged / skipping" shortcut: *what exactly is being compared, and does the consumer check the same thing?*

**Second rule, from the diagnosis cost:** the checksum handler computed the digest, discarded it, deleted the `.part`, and logged only a message — so a wrong manifest and a damaged download were indistinguishable afterwards. Reconstructing the facts needed a 1.26 GB re-download. Any handler that deletes, resets, quarantines or rolls back logs the identifying facts first (which item, which URL, expected vs actual, sizes), and names the failing item on screen: "a timing model" withheld the one fact that would have located it.

## Session 254 (2026-09-10) — a comparison over fields that don't exist reports "all identical"

**What happened:** asked whether Facebook/Instagram/TikTok really receive near-identical text, I wrote a probe over `clipflow-publish-log.json` and reported **"121 of 121 clips have identical caption and title"** — and said so to Fega as a finding. The log's fields are `clipCaption` / `clipTitle`; I read `e.caption` / `e.title`. Every value was `undefined`. `[...new Set(list.map(...).filter(Boolean))]` then produced an empty array, `length <= 1` scored as "identical", and the probe returned a clean, confident, exactly-backwards answer. The truth was **0 of 121** identical — each platform gets its own tailored hashtags. I also had `e.platform !== "youtube"` where the stored value is `"YouTube"`, so the YouTube split was silently empty too.

**Why:** I asserted on the derived aggregate (a count of distinct values) without ever printing a single raw record. A dedupe/set comparison over a missing field doesn't throw and doesn't return zero rows — it collapses to "everything matches", which is a *plausible* answer, so nothing looked wrong. It fails toward the confident conclusion rather than toward an error, which is the worst possible failure direction for a probe whose whole job is to settle a question.

**Rule:** before any aggregate over object fields (dedupe, group-by, count-distinct, diff), print **one raw record** and assert the field names and value casing you are about to key on. If a comparison can return "all identical" when the field is absent, it must first assert the field is present — `filter(Boolean)` hides absence, it does not prove presence. Sibling of s141's "assert on the thing itself, never a proxy you haven't printed" and s246's vacuously-passing negative assertion: same family, all three fail silently toward the answer you expected.

## Session 260 (2026-09-16) — I proposed a "silent fallback" and shrugged at an unlogged path

**What happened:** while planning #419-#421 I described the Claude stills path as a "silent fallback" for Gemini, twice, as if silence were a feature. In the same conversation I told Fega I could not say how often he uses Rephrase/Regenerate or what they cost, because that path writes no cost log and no training row — and I moved on. Fega: "I don't like blind spots... I need things to be tracked and logged." The Claude title path has had zero cost logging since #193 shipped on 2026-07-27, so the monthly total in Settings has been under-reporting for seven weeks and nobody could tell.

**Why:** I treated existing gaps in instrumentation as the landscape rather than as defects. "Silent" felt like good UX (no noise for the user) but it also means no record for the operator — and Fega is the operator who has to find the bottlenecks. A fallback that leaves no trace makes the worse path invisible exactly when it is being used.

**Rule:** every AI call, every fallback, and every user action on an AI output gets a queryable row (DB table or cost log), with the reason when it is a fallback. A fallback is never "silent": it is logged with why, and the surface says in one plain line that the lesser path ran. When I catch myself saying "I can't tell how often X happens", that sentence is a bug report — file it, do not shrug. Sibling of s253's second rule (handlers that discard the facts they computed).

## Session 260 (2026-09-16) — I recommended a cost gate I had never priced

**What happened:** twice I told Fega to leave the generate-on-approve switch off "until the first 30 published rows show #419 moved the take-rate", inheriting Wick's line that auto-generating rejected cards "just spends money faster". At wrap he asked what that even meant and said he wanted titles immediately. Pricing it took one multiplication: he already presses Generate on 63% of clips, so the switch adds ~50 generations a month at 1.5 cents — under a dollar. The gate cost him the feature he asked for, for nothing.
...
## Session 265 (2026-09-18) — new tags in the Layout drawer pushed it 58px past its edge; the #215 rule existed in the ui-debug skill (self-caught in verification)

**What happened:** #444 added "This section" / "This clip" tags to the Saved layouts rows. The first screenshot showed "Paste layou" and half-sentences cut off at the drawer's right edge. Radix ScrollArea wraps the viewport content in `display: table`, which sizes to its widest row, and the nowrap tags widened every row (339px drawer, 407px content). The longest saved-layout name had already overflowed it by 10px. The clamp (`[&_[data-radix-scroll-area-viewport]>div]:!block`, scoped to the one ScrollArea) was written up in clipflow-ui-debug after #215. That skill loads when fixing a UI bug, not when adding content to a panel, so it never fired.
