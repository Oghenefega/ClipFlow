...
> <!-- DISTILLED-THROUGH: 2026-09-21 (s271) -- Session 271: three self-caught lessons, all routed to clipflow-code-review section 5 (No Regressions). (1) Canvas posters of 2560x2880 stills held ~700 MB -> multi-pass memory run vs the <img> baseline, scale stills in main; memory project_canvas_fullsize_still_memory. (2) #450's release always fired from its 2.5 s backstop (the last drag frame lands mid-seek) -> time which of condition/backstop fired, and a lock probe must first show the lock; memory project_video_preview_file_lock updated. (3) Editor compositor has run-to-run edge noise -> pixel A/B needs a same-build baseline. Also memory feedback_commit_hook_wrap_word (the changelog hook blocks any commit command containing "wrap") and project_youtube_shorts_thumbnail_api updated for #454/#455/#452. Everything below the NEXT-UNDISTILLED marker dated 2026-09-21 or earlier is distilled. -->
